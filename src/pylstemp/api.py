...
from .emissivity.registry import emissivity_registry
from .exceptions import InputShapesNotEqual
from .references import ORIGINAL_LIBRARY_CREDIT
from .temperature.registry import single_window_registry, split_window_registry
from .thermal.brightness import brightness_temperature
from .vegetation.ndvi import ndvi
...
def single_window(
    ...
    # We keep the invalid-pixel behavior explicit and centralized.
    ...
    result = single_window_registry.create(lst_method)(
...
def list_algorithms() -> dict[str, dict[str, dict[str, str]]]:
    ...
    return {
        "credit": {"original_library": ORIGINAL_LIBRARY_CREDIT},
        "emissivity": {
            key: {
                "name": metadata.name,
            ...
            for key, metadata in emissivity_registry.describe().items()
        },
        "temperature.single_window": {
            key: {
                "name": metadata.name,
                "reference": metadata.reference,
                "citation": metadata.citation,
            }
            for key, metadata in single_window_registry.describe().items()
        },
        "temperature.split_window": {
            key: {
                "name": metadata.name,
                "reference": metadata.reference,
                "citation": metadata.citation,
            }
            for key, metadata in split_window_registry.describe().items()
        },
    }
